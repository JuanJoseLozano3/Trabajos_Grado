...
def LimpiarUnaVez():
    ...
    root.destroy()
    root = tk.Tk()
    #exec(open("Interfaz/BuscarTrabajo.py").read())

#if __name__ == '__main__':
def Pantalla(Seleccion,Dato):
    root = tk.Tk()
    Seleccion=2
    Dato=2420151030
    ...
    entryNombrePro.insert(0,NomDir)
    ...
    Trabajo = tk.Label(root, text="Trabajo", font=('courier', 10, 'bold'), justify=tk.CENTER)
    ...
    variable.set(Mod)
    opt = tk.OptionMenu(root, variable, *Modalidad)
    ...
    variable = tk.StringVar(root)
    variable.set(Eta)
    opt = tk.OptionMenu(root, variable, *Etapa)
    opt.config(font=('Helvetica', 12))
    ...
    elif (Eta == "Proyecto en desarrollo"):
        Fecha = [
            "Inicio de proyecto"
        ]
    elif (Eta == "Proyecto en prorroga"):
        Fecha = [
            "Inicio de proyecto con prorroga",
            "Fin de proyecto con prorroga"
        ]
    elif(Eta=="Proyecto finalizado"):
        Fecha = [
            "Fin del proyecto",
            "Fin del proyecto con prorroga"
        ]

    variable = tk.StringVar(root)
    variable.set(Fecha[0])
    opt = tk.OptionMenu(root, variable, *Fecha)
    opt.config(font=('Helvetica', 12))
    opt.place(x=20,y=590)

    FechaSel = tk.StringVar()
    entryFechaSel = tk.Entry(root, textvariable=FechaSel)
    # entry.insert(0, "Hola mundo!")
    entryFechaSel.insert(0, "")
    entryFechaSel.config(font=('Helvetica', 12))
    entryFechaSel.place(x=450, y=595)

    Guardar = tk.Button(root, text="Guardar", font=('courier', 15, 'bold'), command=LimpiarUnaVez)
    Guardar.place(x=200, y=630)

    Volver = tk.Button(root, text="Volver", font=('courier', 15, 'bold'), command=LimpiarUnaVez)
